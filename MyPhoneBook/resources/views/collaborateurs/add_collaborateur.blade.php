<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Ajout nouveau Collaborateur') }}
        </h2>
    </x-slot>
        <div style="display: flex; justify-content: space-around;margin:20px 0 0 0">
            <form action="/collaborateur/create/success" method="POST" style="width: 500px;background-color: white;">
                {{ csrf_field() }}
                <fieldset style="border:solid;padding:20px;border-radius:20px">
                    ...
                    <input type="text" name="prenom" id="prenom" required value="{{ old('prenom') }}"><br>
                    ...
                    <input type="tel" name="telephone" id="telephone" oninput="numcheck(this)" onkeydown="numcheck(this)" value="{{ old('telephone') }}"><br>
                    <label for="email">Email</label><br>
                    <input type="email" name="email" id="email" required value="{{ old('email') }}"><br>
                    <label for="entreprise">Entreprise</label><br>
                    <input type="text" name="entreprise" id="entreprise" required value="{{ old('entreprise') }}"><br>
